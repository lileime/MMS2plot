...
calculate_AAmz_individual_label <-function(aa_mw_mod_table, seq, charge, Monoisotopicmz, ppm, flag){
  ...
  if(nrow(AA_mz_NA)>0){
    ...
    stop(paste("The modification(s) [ ", mod_NA_weight, " ] in '", seq_origin,
               "' is not included in the modification.xml file! [note:stopped
               in the function calculate_AAmz_individual_label].", sep=""))
  }

  # for reportor ion (e.g. TMT)
  if(flag == "reporterion"){
    AA_mz_final_start <- subset(AA_mz, AA_mz$reporterion %in% "anyNterm" & AA_mz$index == 1)
    if(nrow(AA_mz_final_start)>0){
      AA_mz_final_others <- subset(AA_mz, is.na(AA_mz$reporterion)       & AA_mz$index != 1)
      AA_mz <- rbind(AA_mz_final_start, AA_mz_final_others)
    }
  }else if(flag == "labelmod"){
    AA_mz_final_start <- subset(AA_mz, AA_mz$labelmod %in% "anyNterm" & AA_mz$index == 1)
    if(nrow(AA_mz_final_start)>0){
      AA_mz_final_others <- subset(AA_mz, is.na(AA_mz$labelmod)       & AA_mz$index != 1)
      AA_mz <- rbind(AA_mz_final_start, AA_mz_final_others)
    }
...
test_individualIon<-function(mz_intensity_percent, AA_mz, b_ion_col, y_ion_col){
  ...
  if(nrow(ion) == 1 && ion$index < max(AA_mz$index)){ # mz matched and mz is smaller than the mw of the AA sequence (using index)
    ion_info <- data.table::data.table("mz"=MIP$mz, "index" = ion$index, "intensity"=MIP$intensity, "intensity_perc" =  MIP$intensity_perc,
                          "abs_intensity_prc_ext" = ifelse(abs(MIP$intensity_perc)>0.30, abs(MIP$intensity_perc), stats::runif(sum(abs(MIP$intensity_perc)<0.30),min=0.30, max=0.65)),
  ...
  if(nrow(ion) == 1 && max(AA_mz$index)-ion$index+1 < max(AA_mz$index) ){ # mz matched and mz is smaller than the mw of the AA sequence (using index)
    yion <- data.table::data.table("mz"=MIP$mz, "index" = ion$index, "intensity"=MIP$intensity, "intensity_perc" =  MIP$intensity_perc,
                      "abs_intensity_prc_ext" = ifelse(abs(MIP$intensity_perc)>0.7, abs(MIP$intensity_perc), stats::runif(sum(abs(MIP$intensity_perc)<0.7), min=0.7, max=1)),
...
find_matchedIons<-function(AA_mz, mz_intensity_percent, b_ion_col, y_ion_col){
  ...
  abs_intensity_prc_ext_max <- stats::aggregate(intensity~ionLabel, data=psm, max)
  ...
  ionLabel_comb <- stats::aggregate(ionLabel~abs_intensity_prc_ext+mz+intensity_perc, psm, paste, collapse=",")
  psm<-merge(ionLabel_comb, psm, by=c("abs_intensity_prc_ext", "mz","intensity_perc"), suffixes = c("_peak", ""))
  if(any(grepl(",", psm$ionLabel_peak))){
    psm[grepl(",", psm$ionLabel_peak),]$col = DescTools::MixColor(y_ion_col, b_ion_col)
  }
  #browser()
  return(psm)
}
...
get_intensity_perc<-function(input_table, min_intensity_ratio){
  ...
  get_peakswoNoise<-function(input_table, min_intensity_ratio){
    ...
    peaks<- data.table::data.table(mz        = as.numeric(unlist(strsplit(input_table["mz"],";"))),
                                  intensity = as.numeric(unlist(strsplit(input_table["intensity"],";"))))
    peaks_wo_Noise<- subset(peaks, peaks$intensity > max_intensity * min_intensity_ratio) # remove intensity < 1% of max_intensity
