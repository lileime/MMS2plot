...
label_axis<-function(max_intensity, xlab, ylab, max_mz, cex,lwd){
  if(length(max_intensity) == 2){ # for mirror_plot
    if( max_intensity[1] > 0){
      axis_y_positive <- seq(0, max_intensity[1], length.out = 5) # quarter of the max_intensity
      axis_y_negative <- seq(max_intensity[2], 0, length.out = 5) # quarter of the max_intensity
    }else{
      axis_y_positive <- seq(max_intensity[1], 0, length.out = 5) # quarter of the max_intensity
      axis_y_negative <- seq(0, max_intensity[2], length.out = 5) # quarter of the max_intensity
    }
    ...
    axis(side = 2,las = 1, at = seq(-1,1, length.out = 9), labels = as.integer(axis_y), tck=-0.01, mgp=c(0, 0.2, 0), cex.axis=0.33, lwd=0.5*lwd)# format(axis_y_positive,scientific=FALSE))
    ...
    axis(side = 2,las = 1, at = seq(0, 1, length.out = 5), labels = as.integer(axis_y), tck=-0.02, mgp=c(0, 0.2, 0), cex.axis=0.33, lwd=0.5*lwd)# format(axis_y_positive,scientific=FALSE))
...
draw_peak_ionL<-function(psm, lwd, len_annoSpace, srt,show_letterBY){
  #browser()
  ion_unique<-unique(psm, select=c("mz", "abs_intensity_prc_ext", "direction", "intensity_perc","ionLabel_peak","col"))
  lines(ion_unique$mz, ion_unique$intensity_perc, type = "h",las=1, col=ion_unique$col, lwd = lwd/2)
  segments(ion_unique$mz,ion_unique$intensity_perc, ion_unique$mz, ion_unique$abs_intensity_prc_ext*ion_unique$direction, col=ion_unique$col, lwd = lwd/2, lty=3) # draw peak extention segments
  ion_name <- substr(ion_unique$ionLabel_peak, 1, regexpr(pattern ='\\+',ion_unique$ionLabel_peak)-1)
  number_plus <- substring(ion_unique$ionLabel_peak, regexpr(pattern ='\\+',ion_unique$ionLabel_peak))
  if(show_letterBY){
    text( ion_unique$mz, (ion_unique$abs_intensity_prc_ext + len_annoSpace) * ion_unique$direction,
          labels <- mapply(function(x,y)  as.expression(bquote(.(x)^.(y))), ion_name, number_plus), cex = 0.3, col=ion_unique$col, adj=0.5, srt=0) # b/y ion,e.g.b3+
  }else{
    text( ion_unique$mz, (ion_unique$abs_intensity_prc_ext + len_annoSpace) * ion_unique$direction,
          labels <- mapply(function(x,y)  as.expression(bquote(.(x)^.(y))), substring(ion_name,2), number_plus), cex = 0.3, col=ion_unique$col, adj=0.5, srt=0) # b/y ion,e.g.b3+
  }
}
...
print_modpeptide <- function(PSMlabel, peptide_height, mod_height, pos_start_dash, pos_end_dash, direction){
  #browser()
  peptide_height <- peptide_height * direction
  mod_height  <- mod_height *  direction
  if(PSMlabel$peptide == "."){
    return(NULL)# ignore
  }else if(grepl("^[A-Z]",PSMlabel$peptide) || PSMlabel$peptide == "-"){ # AA at the front
    if( nchar(PSMlabel$peptide) == 1 ){  # without modification
      text(PSMlabel$AA_pos, peptide_height, PSMlabel$peptide, cex=0.5, adj=0.5)
    }else{ # with modification at the end
      AA <- substr(PSMlabel$peptide,1,1)
      AA_endMod <- substr(PSMlabel$peptide,3, nchar(PSMlabel$peptide)-1)
      text(PSMlabel$AA_pos, peptide_height,                 bquote(.(AA)),           cex = 0.5, adj=0.5)
      if(direction >0){
        text(PSMlabel$AA_pos, peptide_height + mod_height, bquote(""[.(AA_endMod)]),cex = 0.5, adj=0.5)
      }else{
        text(PSMlabel$AA_pos, peptide_height - mod_height, bquote(""[.(AA_endMod)]),cex = 0.5, adj=0.5)
      }
    }
  }else if(grepl("^\\(",PSMlabel$peptide)){ # mod at the start
    #browser()
    bracket_start <- gregexpr("(", PSMlabel$peptide,fixed=TRUE)[[1]]
    if(length(bracket_start) == 1){  # only one mod which is at the start
      AA_frontMod <- substr(PSMlabel$peptide, 2, nchar(PSMlabel$peptide)-2)
      AA <- substring(PSMlabel$peptide, nchar(PSMlabel$peptide))
      if(direction > 0){
        text(pos_start_dash, peptide_height + mod_height, bquote(""[.(AA_frontMod)]), cex = 0.5, adj=0.5)
        text(PSMlabel$AA_pos, peptide_height, bquote(.(AA)),cex = 0.5, adj=0.5)
      }else{
        text(pos_start_dash, peptide_height - mod_height, bquote(""[.(AA_frontMod)]), cex = 0.5, adj=0.5)
        text(PSMlabel$AA_pos, peptide_height, bquote(.(AA)),cex = 0.5, adj=0.5)
      }
    }else if(length(bracket_start) == 2){  # two mod: one at the front and another at the end
      AA_frontMod <- substr(PSMlabel$peptide, 2, bracket_start[2]-3)
      AA <- substr(PSMlabel$peptide, bracket_start[2]-1, bracket_start[2]-1)
      AA_endMod <- substr(PSMlabel$peptide, bracket_start[2]+1, nchar(PSMlabel$peptide)-1)
      text(PSMlabel$AA_pos, peptide_height,  bquote(.(AA)), cex = 0.5, adj=0.5)
      if(direction > 0){
        text(pos_start_dash,  peptide_height + mod_height, bquote(""[.(AA_frontMod)]), cex = 0.5, adj=0.5)
        text(PSMlabel$AA_pos, peptide_height + mod_height, bquote(""[.(AA_endMod)]) ,  cex = 0.5, adj=0.5)
      ...
      stop("CAN NOT BE THREE MODS! [note:stopped in the function print_modpeptide].")
    }
  }else{
    stop("CAN NOT BE THREE MODS! [note:stopped in the function print_modpeptide].")
  }
...
draw_ms2generalinfo<-function(rt, scan, mz, charge, gene, PSM, info_height){
  ...
  text(25, info_height*direction, paste(scan, mz, charge, rt, gene, sep="; "), pos=4, offset=0, cex = 0.33)
}
...
plot_group_individual<-function(mz_intensity_percent, AA_mz,  PSM, max_intensity, base_rawFile, rt, scan, mz, charge, gene,
  ...
  draw_psmanno( AA_mz, PSM, max_mz, peptide_height, mod_height, len_annoSpace, y_ion_col, b_ion_col, lwd=lwd/2 )
