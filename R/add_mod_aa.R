...
add_mod_aa<-function(mod_parameter_xml, MS2FileName, aa_mw_table, mqpar_ppm){
  ...
  if(!mqpar$fixedModifications %in% empty ) { # contains fixed modifications
    ...
    modifications <- mapply(extract_mod_xml, Mod, MoreArgs=list(mod_parameter_xml), SIMPLIFY=FALSE) # Mod="Carbamidomethyl (C)"
    modifications <- do.call(rbind, modifications) #

    # for modifications occurring dependent of specific AA
    aa_mw_table_fixed <- subset(aa_mw_table, aa_mw_table$aa %in% modifications$mod_aa)
    if(nrow(aa_mw_table_fixed)>0){
      ...
      aa_mw_table <- rbind( subset(aa_mw_table, ! aa_mw_table$aa %in% modifications$mod_aa), aa_mw_table_fixed)
  ...
  if(!mqpar$variableModifications %in% empty ) { # contains variable modifications
    ...
    modifications <- mapply(extract_mod_xml, Mod, MoreArgs=list(mod_parameter_xml), SIMPLIFY=FALSE) # Mod="Phospho (STY)"
    ...
    # for modifications occurring dependent of specific AA
    aa_mw_table_mod <- subset(aa_mw_table, aa_mw_table$aa %in% modifications$mod_aa)
  ...
  if(!mqpar$isobaricLabels %in% empty ){ # contains reporter ion, e.g. TMT, iTRAQ; TMT10plex-Nter128,TMT10plex-Nter130,TMT10plex-Lys128,TMT10plex-Lys130
    ...
    Mods<-stats::aggregate(Mods$title, by=list(Mods$mw), FUN=function(x) paste(x, collapse=";"), simplify=FALSE)$x
...
extract_mod_xml <-function(Mod, xmlurl){
  ...
  mod_detail <- data.table::data.table(cbind(Mod,mod_abb,mod_comp_mw, mod_aa, mod_pos),stringsAsFactors=FALSE)
  return(mod_detail)
}
...
calculate_aa_wm_anyTerminal<-function(x, aa_mw_mod_table, flag, tmp=NA){
  ...
  if(flag == "variableModifications"){
    mod_anypos_left <- data.table::data.table(aa = mod_anypos$aa, weight = as.numeric(mod_anypos$mod_comp_mw) + as.numeric(mod_anypos$weight),
...
calculate_aa_wm_label <-function(Mod, mod_parameter_xml, aa_mw_mod_table, flag){
  ...
  if(Mod != ""){
    ...
    modifications <- mapply(extract_mod_xml, Mod_invidual, MoreArgs=list(mod_parameter_xml), SIMPLIFY=FALSE) # Mod_invidual="Phospho (STY)"
    modifications <- do.call(rbind, modifications)
    #browser()

    # for modifications occurring dependent of AA
    aa_mw_mod_table_label <- subset(aa_mw_mod_table, aa_mw_mod_table$aa %in% modifications$mod_aa)
    if(nrow(aa_mw_mod_table_label)>0){
      ...
      aa_mw_mod_table_unlabel <- subset(aa_mw_mod_table, ! aa_mw_mod_table$aa %in% modifications$mod_aa)
